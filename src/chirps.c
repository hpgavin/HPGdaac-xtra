...
Program chirps.c - Generate a file of sinusoidal data with changing frequency and amplitude
...
gcc -O -o chirps chirps.c HPGutil.c NRutil.c -lm 
...
{
  char  fn[80],            // the output sine-sweep filename
        usage[128];

  FILE *fp;                // file pointer to the output data file

  float  sr = 200.0,       // the sampe rate    
         offset = 0.0,     // static offset "set point"  
         phase = 0.0,      // the instantaneous phase angle
         ao = 50,          // starting amplitude
         af = 50,          // stopping amplitude  
         fo =  0.5,        // starting frequency limit
         ...
         ampl,             // the current amplitude
        *u;                // vector of sine-sweep data

  int    nTu, nTd,         // points in the taper stages
         nTs,              // number of points in sine sweep
  ...
  sprintf(usage," chirps sr offset ao af fo ff T p q outFile");

  printf("\n  to run:  %s \n\n", usage );

  // default values 
  if (argc < 11) sprintf(fn,"chirp.dat"); else strcpy( fn, argv[10]);
